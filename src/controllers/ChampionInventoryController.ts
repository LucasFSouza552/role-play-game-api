...
export class ChampionInventoryController implements ControllerInterface {
    ...
    async getById(req: Request, res: Response): Promise<void> {
        ...
        try {
            const inventory = await inventoryService.getById(championId, userId);
    ...
    async createItemInventory(req: Request, res: Response): Promise<void> {
        try {
            ...
            const inventory = await inventoryService.createItemInventory(userId, championId, itemId, quantity);
            res.status(200).json(inventory);
        } catch (err: any) {
            res.status(400).json({ error: err.message });
        }
    ...
    async removeItemInventory(req: Request, res: Response): Promise<void> {
        try {
            ...
            if (!item || isNaN(item) || !quantity || isNaN(quantity) || quantity <= 0) {
                res.status(400).json({ error: "Invalid item or quantity" });
                return;
            }

            const inventoryExists = await inventoryService.getById(championId, userId);
            if (!inventoryExists) {
                res.status(404).json({ error: "Inventory not found" });
                return;
            }

            if (inventoryExists.ownerId !== championId) {
                res.status(403).json({ error: "Access to this inventory is forbidden" });
                return;
            }

            // const updatedInventory = await inventoryService.removeItemInventory(inventoryId, item, quantity);
